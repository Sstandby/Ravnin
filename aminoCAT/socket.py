...
from .lib.util.headers import sigg
...
class SocketHandler:
    ...
    def run_amino_socket(self):
        ...
        mili = int(time.time() * 1000)
        data = f"{self.client.device_id}|{mili}"
        self.headers["NDC-MSG-SIG"] = sigg(data)

        self.socket = websocket.WebSocketApp(
            f"{self.socket_url}/?signbody={self.client.device_id}%7C{mili}",
