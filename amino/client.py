...
class Client(Callbacks, SocketHandler):
    def __init__(self, deviceId: str = "2271017D5F917B37DAC9C325B10542BC9B63109292D882729D1813D5355404380E2F1A699A34629C10", proxies: dict = None, certificatePath = None, socket_trace = False, socketDebugging = False):
        ...
        self.adbsd = headers.AdBsd().headers
        self.adbsd_data = headers.AdBsd().data
        self.json = None
        self.sid = None
        self.userId = None
    ...
    def watch_ad(self, uid: str = None):
        if uid: self.adbsd_data["reward"]["custom_json"]["hashed_user_id"] = uid
        if not uid: self.adbsd_data["reward"]["custom_json"]["hashed_user_id"] = self.uid
        self.adbsd_data["reward"]["event_id"] = str(uuid4())
        response = requests.post("https://ads.tapdaq.com/v4/analytics/reward", headers=self.adbsd, json=self.adbsd_data)
        return response.status_code
